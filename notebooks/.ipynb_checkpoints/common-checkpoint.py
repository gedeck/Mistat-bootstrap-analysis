...
def saveFigure(filename):
    ...
    content = re.sub(br'\/CreationDate \(D:\d+\D*\)', b'/CreationDate (D:20230823000000Z)', content)
    content = re.sub(br'\/ModDate \(D:\d+\D*\)', b'/ModDate (D:20230823000000Z)', content)
...
def preparePistonDataset() -> pd.DataFrame:
    ...
    return result[['seconds', 'group', 'x1', 'x2', 'x3', 'x4']]
...
def parametricBootstrapAnalysis(df: pd.DataFrame, formula: str, nrepeats: int = NREPEATS_DEFAULT) -> pd.DataFrame:
    ...
    results = []
    for _ in range(nrepeats):
        df[outcome] = model.fittedvalues + residuals.sample(frac=1, replace=True).reset_index(drop=True)
...
def plot_coefficients(ols_model, bba_samples, ba_samples, pbba_samples, pba_samples, wba_samples):
    terms = list(ols_model.params.index)
    ncols = 6
    if len(terms) > ncols:
        nrows = (len(terms) - 1) // ncols + 1
        fig, axes = plt.subplots(ncols=ncols, nrows=nrows, figsize=[16, 3 * nrows])
        axes = itertools.chain(*axes)
    else:
        fig, axes = plt.subplots(ncols=ncols, nrows=1, figsize=[16, 3])
    ...
    for ax, term in itertools.zip_longest(axes, terms):
        ...
        ax.set_xticks([0, 1, 2, 3, 4, 5], ['OLS', 'BBA', 'BA', 'pBBA', 'pBA', 'wBA'])
        ax.set_xlim(-0.5, 5.5)
        ax.axhline(0)
